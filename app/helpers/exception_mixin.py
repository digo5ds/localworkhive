"""This module provides utilities for handling common exceptions when
interacting with AWS services using boto3/botocore."""
...
class BotoExceptionHandlingMixin:
    """A mixin class that provides a method to safely execute functions
    interacting with AWS services, handling common boto3 exceptions."""

    def safe_execute(self, func, *args, **kwargs):
        """Executes a function safely by catching and handling specific AWS-
        related exceptions.
        ...
        try:
            ...
            raise ValueError(
                f"An error occurred while executing the function: {str(e)}"
            ) from e
...
def exception_safe(method):
    ...
    The wrapper calls the wrapped method with the provided arguments and
    keyword arguments, and catches any exceptions of type ClientError,
    EndpointConnectionError, NoCredentialsError, and
    PartialCredentialsError. If an exception is caught, it is printed to
    the console and re-raised.

    :param self: The instance of the class containing the wrapped
        method.
    ...
    :param kwargs: The keyword arguments to be passed to the wrapped
        method.
    :return: The result of calling the wrapped method, or None if an
